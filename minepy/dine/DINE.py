...
from minepy.dine.dine_tools_org import UniformFlow
...
class DualCNF(LightningModule):
    def __init__(
        self, dx, dy, dz, n_components, hidden_sizes, lr, weight_decay, verbose
    ):
        super().__init__()
    ...
    def configure_optimizers(self):
        ...
        scheduler = ReduceLROnPlateau(
            optimizer, factor=0.1, patience=10, verbose=self.hparams.verbose
        )
        return {
            "optimizer": optimizer,
    ...
    def fit(self, X, Y, Z, max_epochs, verbose, gpus=None, callbacks=None):
        ...
        with warnings.catch_warnings():
            ...
            train, valid = random_split(
                TensorDataset(X, Y, Z), lengths=[train_size, valid_size]
            )
            train_dataloader = DataLoader(train, batch_size=64, shuffle=True)
            val_dataloader = DataLoader(valid, batch_size=64)
            early_stopping = EarlyStopping(
                mode="min", monitor="val_loss", patience=10, verbose=verbose
            )
            ...
            (logs,) = trainer.validate(
                model=self, dataloaders=val_dataloader, verbose=verbose
            )
